/**
 * File: homepage-enhanced-icons.tsx
 * Purpose: Homepage với biểu tượng icon thử đồ hợp lý hơn
 * 
 * Thay đổi:
 * - Thay Sparkles → Shirt (áo) cho nút "Thử đồ"
 * - Thêm biểu tượng phù hợp cho từng section
 * - Giữ animations và styling hiện tại
 */
...
import {
  ...
  Share2,
  Eye,
  Shirt,          // NEW: Biểu tượng áo cho thử đồ
  Camera,         // NEW: Biểu tượng camera
  Palette,        // NEW: Biểu tượng bảng màu
  Wand2,          // NEW: Biểu tượng phép thuật AI
} from 'lucide-react';
  ...
  return (
    <div className="flex-shrink-0 w-32 rounded-xl overflow-hidden bg-[var(--card)] border border-[var(--border)] shadow-sm hover:shadow-md transition-all duration-200 group">
      <div className="relative aspect-[3/4] bg-gradient-to-br from-orange-400/10 to-pink-500/10">
        ...
        {/* Try button - NEW ICON: Shirt instead of Play */}
        <button
          onClick={onTry}
...
function GridCard({ item, onLike, onSave, onComment }: {
  item: OutfitItem;
  onLike: () => void;
  onSave: () => void;
  onComment: () => void;
  ...
  return (
    <div className="rounded-xl overflow-hidden bg-[var(--card)] border border-[var(--border)] shadow-sm hover:shadow-md transition-all duration-200 group">
      <div className="relative aspect-[3/4] bg-gradient-to-br from-orange-400/10 to-pink-500/10">
        ...
        {/* Try button - NEW ICON: Shirt for try-on */}
        <Link
          href="/tryon"
        ...
        <section className="py-3">
          ...
          <div className="px-3">
            <div className="flex gap-2 overflow-x-auto scrollbar-hide">
              {/* NEW Try-On Button - Enhanced with Shirt icon */}
              <Link
                href="/tryon"
                className="flex-shrink-0 w-16 h-24 rounded-xl border-2 border-dashed border-orange-500/50 bg-orange-500/5 flex flex-col items-center justify-center gap-1 hover:bg-orange-500/10 transition-colors group"
              >
              ...
              {!historyLoading && history.length === 0 && (
                <div className="flex-shrink-0 w-32 h-24 rounded-xl bg-[var(--muted)]/30 border border-dashed border-[var(--border)] flex items-center justify-center">
                  <div className="text-center">
                    <Camera className="w-6 h-6 text-[var(--muted-foreground)] mx-auto mb-1" />
            ...
            {forYouLoading ? (
              ...
              displayForYou.map((item) => (
                <GridCard
                  ...
                  onLike={() => { }}
                  onSave={() => { }}
                  onComment={() => setOpenCommentsFor(item.id)}
                />
              ))
            )}
          </div>
